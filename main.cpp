#include "GaussSolver.h"

#define CONST_N 3
#define CONST_M 2

void main() 
{
	Matrix a(3, 2);
	Vector b(3);
	GaussSolver sol;
	a.resize(3, 2);
	b.resize(3);
	a(0, 0) = 1; a(0, 1) = 1; //a(0, 2) = 1;
	a(1, 0) = 0; a(1, 1) = 1; //a(1, 2) = -1;
	a(2, 0) = 0; a(2, 1) = 1; //a(2, 2) = 1;
	b[0] = 3; b[1] = 1; b[2] = 0;
	sol.solve(a, b); //
	//
	a.resize(3, 2);
	b.resize(3);
	a(0, 0) = 1; a(0, 1) = 1; 
	a(1, 0) = 0; a(1, 1) = 1; 
	a(2, 0) = 0; a(2, 1) = 0; 
	b[0] = 3; b[1] = 1; b[2] = 0;
	sol.solve(a, b); //2, 1
	//
	a.resize(2, 3);
	b.resize(2);
	a(0, 0) = 1; a(0, 1) = 2; a(0, 2) = 1;
	a(1, 0) = 3; a(1, 1) = 1; a(1, 2) = -1;
	b[0] = 1; b[1] = 1;
	sol.solve(a, b); //1/5, 2/5, 0
	//-0.6 0.8 1
	//
	a.resize(3, 3);
	b.resize(3);
	a(0, 0) = 2; a(0, 1) = 4; a(0, 2) = 1;
	a(1, 0) = 5; a(1, 1) = 2; a(1, 2) = 1;
	a(2, 0) = 2; a(2, 1) = 3; a(2, 2) = 4;
	b[0] = 36; b[1] = 47; b[2] = 37;
	sol.solve(a, b); //7, 5, 2
	//
	a.resize(3, 3);
	b.resize(3);
	a(0, 0) = 1; a(0, 1) = 1; a(0, 2) = 0;
	a(1, 0) = 0; a(1, 1) = 1; a(1, 2) = 0;
	a(2, 0) = 0; a(2, 1) = 0; a(2, 2) = 0;
	b[0] = 3; b[1] = 1; b[2] = 0;
	sol.solve(a, b); //2, 1, 0
	//0 0 1
	//
	a.resize(3, 2);
	b.resize(3);
	a(0, 0) = 1; a(0, 1) = 1; //a(0, 2) = 0;
	a(1, 0) = 0; a(1, 1) = 0; //a(1, 2) = 0;
	a(2, 0) = 0; a(2, 1) = 0; //a(2, 2) = 0;
	b[0] = 3; b[1] = 1; b[2] = 0;
	sol.solve(a, b); //
	//
	a.resize(3, 3);
	b.resize(3);
	a(0, 0) = 1; a(0, 1) = 2; a(0, 2) = 3;
	a(1, 0) = 4; a(1, 1) = 5; a(1, 2) = 6;
	a(2, 0) = 7; a(2, 1) = 8; a(2, 2) = 9;
	b[0] = 0; b[1] = 0; b[2] = 0;
	sol.solve(a, b); //0 0 0
	//-1 2 1
	//
	a.resize(3, 2);
	b.resize(3);
	a(0, 0) = 1; a(0, 1) = 2; //a(0, 2) = 3;
	a(1, 0) = 4; a(1, 1) = 5; //a(1, 2) = 6;
	a(2, 0) = 7; a(2, 1) = 8; //a(2, 2) = 9;
	b[0] = 0; b[1] = 0; b[2] = 0;
	sol.solve(a, b); //0 0
	//
	a.resize(2, 3);
	b.resize(2);
	a(0, 0) = 1; a(0, 1) = 2; a(0, 2) = 3;
	a(1, 0) = 4; a(1, 1) = 5; a(1, 2) = 6;
	//a(2, 0) = 7; a(2, 1) = 8; a(2, 2) = 9;
	b[0] = 0; b[1] = 0; //b[2] = 0;
	sol.solve(a, b); //0 0 0
	//-1 2 1
	//
	a.resize(4, 4);
	b.resize(4);
	a(0, 0) = 1; a(0, 1) = 1; a(0, 2) = 0; a(0, 3) = 0;
	a(1, 0) = 0; a(1, 1) = 1; a(1, 2) = 0; a(1, 3) = 0;
	a(2, 0) = 0; a(2, 1) = 0; a(2, 2) = 0; a(2, 3) = 0;
	a(3, 0) = 0; a(3, 1) = 0; a(3, 2) = 0; a(3, 3) = 0;
	b[0] = 3; b[1] = 1; b[2] = 0; b[3] = 0;
	sol.solve(a, b); //2, 1, 0, 0
	//
	a.resize(4, 4);
	b.resize(4);
	a(0, 0) = 1; a(0, 1) = 0; a(0, 2) = 1; a(0, 3) = 0;
	a(2, 0) = 0; a(2, 1) = 0; a(2, 2) = 1; a(2, 3) = 0;
	a(1, 0) = 0; a(1, 1) = 0; a(1, 2) = 0; a(1, 3) = 0;
	a(3, 0) = 0; a(3, 1) = 0; a(3, 2) = 0; a(3, 3) = 0;
	b[0] = 3; b[1] = 1; b[2] = 0; b[3] = 0;
	sol.solve(a, b); //2, 0, 1, 0
	a.resize(2, 5);
	b.resize(2);
	a(0, 0) = 1; a(0, 1) = 2; a(0, 2) = 0; a(0, 3) = 3; a(0, 4) = 3;
	a(1, 0) = 4; a(1, 1) = 5; a(1, 2) = 0; a(1, 3) = 6; a(1, 4) = 6;
	//a(2, 0) = 7; a(2, 1) = 8; a(2, 2) = 9;
	b[0] = 0; b[1] = 0; //b[2] = 0;
	sol.solve(a, b); //0 0 0 0 0
	//-1 2 0 1 0
	//-1 2 0 0 1
	a.resize(5, 5);
	b.resize(5);
	a(0, 0) = 1; a(0, 1) = 2; a(0, 2) = 0; a(0, 3) = 3; a(0, 4) = 3;
	a(1, 0) = 0; a(1, 1) = 0; a(1, 2) = 0; a(1, 3) = 0; a(1, 4) = 0;
	a(2, 0) = 0; a(2, 1) = 0; a(2, 2) = 0; a(2, 3) = 0; a(2, 4) = 0;
	a(3, 0) = 4; a(3, 1) = 5; a(3, 2) = 0; a(3, 3) = 6; a(3, 4) = 6;
	a(4, 0) = 0; a(4, 1) = 0; a(4, 2) = 0; a(4, 3) = 0; a(4, 4) = 0;
	b[0] = 0; b[1] = 0; b[2] = 0; b[1] = 0; b[2] = 0;
	sol.solve(a, b); //0 0 0 0 0
	//-1 2 0 1 0
	//-1 2 0 0 1
}